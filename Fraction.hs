...
reduce  :: Fraction -> Fraction     -- Fraction reducer.
reduce (num:/0) =                   -- Denominator equals zero: do anything.
    if num < 0 
    then (-1):/0
    else 1:/0

reduce (num:/den) =                 -- Simplification.
    (quot u d):/(quot v d)
    where
        d      = gcd u v            -- Great common divisor.
        (u, v) =
            if den < 0 
            then (-num, -den)
            else (num, den)

(//) :: Integer -> Integer -> Fraction -- Fraction common reader (includes reduce)
